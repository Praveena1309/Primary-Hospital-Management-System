...
def pharmacist_history(request):
    if request.user.is_hospital_admin:
        prescription = Prescription.objects.all().order_by('-prescription_id')
    
    context = {'prescription':prescription,}
        
    return render(request, 'hospital_admin/pharmacist-history.html',context)
@csrf_exempt
def prescription_view_admin(request,pk):
      if request.user.is_hospital_admin:
        prescription = Prescription.objects.filter(prescription_id=pk)

        context = {'prescription':prescription,}
        return render(request, 'hospital_admin/prescription_view_admin.html',context)
      else:
         redirect('logout') 
...
def update_prescription_status(request,pk):
    ...
    if request.user.is_pharmacist :
        ...
        messages.success(request, 'Prescription status updated to Completed')
        return redirect('pharmacist-dashboard')
    else:
        # If user is not authorized, redirect back to the previous page or show an error message
        messages.error(request, 'You are not authorized to update the status of this prescription')
        return redirect('pharmacist-dashboard')
@csrf_exempt
