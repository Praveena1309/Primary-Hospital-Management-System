...
from .models import Doctor_Information, Patient, Prescription
@login_required(login_url="login")
def patient_search(request, pk):
    if request.user.is_authenticated and request.user.is_doctor:
        try:
            doctor = Doctor_Information.objects.get(doctor_id=pk)
            id = int(request.GET.get('search_query', ''))
            ...
            # Patient does not exist, render a template indicating this
            return render(request, 'patient-not-found.html')
    else:
        logout(request)
